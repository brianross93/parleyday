...
def _player_profile(player_id: str, team_code: str, slot: str, values: dict[str, float]) -> PlayerSimProfile:
    positions = {"pg": ("PG",), "sg": ("SG",), "sf": ("SF",), "pf": ("PF",), "c": ("C",)}[slot]
...
def _rotation_plan(players: tuple[PlayerSimProfile, ...]) -> RotationPlan:
    starters = tuple(player.player_id for player in players)
    return RotationPlan(
        starters=starters,
        closing_group=starters,
        target_minutes={player.player_id: 34.0 for player in players},
        max_stint_minutes={player.player_id: 10.0 for player in players},
        backup_priority={player.player_id: () for player in players},
    )
